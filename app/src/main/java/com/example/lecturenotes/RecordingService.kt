...
class RecordingService : Service() {
    ...
    companion object {
        ...
        // Flow для передачи аудио-чанков подписчикам (MainActivity)
        ...
        // Метод для отправки чанка (используется внутри Service)
        internal fun emitChunk(chunk: ByteArray) {
            ...
            FileOutputStream(outputFile).use { fos ->
                ...
                while (isRecording.get()) {
                    ...
                    if (read > 0) {
                        ...
                        // Отправляем чанк в Flow (вместо AudioBuffer)
                        emitChunk(chunk)
                    }
                }
            }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "Запись лекций", NotificationManager.IMPORTANCE_LOW)
